...
$.get('map.txt', function(csvString) {
    var data = Papa.parse(csvString, {header: true, dynamicTyping: true, keepEmptyRows:false, skipEmptyLines: true,delimiter: "|",}).data;

	// create empty filter object with gateways and dates as properties to filter in map 
	// todo : future extensions (min/max rssi for example to create better legend)

	var gw = {};

	// Optional : get first gateway location for centering map, beter set it directly to the right center when initialising (faster)
    ...
    for (var i in data) {
    	...
    	// collect and push into filtergroup
		var filterarray = [];
		for (var gwprop in gw) {filterarray.push(`${gwprop}`);}
		filterarray.sort();
